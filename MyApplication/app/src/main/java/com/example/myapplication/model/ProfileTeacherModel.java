...
//        System.out.println("ProfileTeacherModel + checkInforValidity "+url);
...
//                            System.out.println("Real class: " + iProfileTeacherView.getClass().getName());
                            /// --> Turn to ProfileTeacherActivity (Line 159)
    ...
    public void checkInforValidityMain(String id, IProfileTeacherView iProfileTeacherView) {
        String url = "http://" + ipConfigModel.getIpconfig() + "/PHP_API/inforteacher.php";
//        System.out.println("ProfileTeacherModel + checkInforValidityMain "+url);
        RequestQueue requestQueue = Volley.newRequestQueue((Context) iProfileTeacherView);
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                if (response.equals("Error")) {
                    ...
                    try {
                        JSONObject object = new JSONObject(response);
                        if ((object.getString("teacher_id").trim()).equals(id)) {
                            /// --> Turn to TeacherActivity (Line 130)
